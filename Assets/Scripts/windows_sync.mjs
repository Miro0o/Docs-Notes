...
export async function sync({ source, target, config, stateFile = STATE, head: requestedHead, base: requestedBase, baseline: requestedBaseline }) {
  ...
  if (remaining.issues.some(i => ['windows-rename', 'encoded-separator'].includes(i.category) && !i.drawingText)) throw new Error('Converted links failed verification; sync state was not advanced');
