# English Grammar

[TOC]



## Res
### Related Topics


### Other Resources
📖《新编英语语法教程》 - 新世纪高等院校英语专业本科生系列教材
![|400](../../../../../../Assets/Pics/49bb5539d6b84958.jpg)
<small>https://item.jd.com/13254459.html</small>

📖 the origin of names, words, and everything in between



## Intro



## English Grammatical Hierarchy
### 1️⃣ Morpheme
↗ [Morpheme & Word](Morpheme%20&%20Word/Morpheme%20&%20Word.md)
↗ [Morpheme-Word Reference -- 📿 miscs](Morpheme%20&%20Word/Morpheme-Word%20Reference/📿%20miscs.md)

词素是最小的语法单位，也是最小的语义单位。词素分为两大类：自由词素和粘附词素。
- 自由词素（Free Morpheme）：可以独立作为简单词，或若干自由词素组合在一起组成复合词，或充当词根（root）与词缀构成派生词。
	- boy, girl, desk, chair
	- book + mark: book mark, take + over = takeover
	- kind: kindness, unkind, kindly
- 粘附词素（Bound Morpheme）：不可单独存在，必须粘附在其他词素形式上。
	- 词缀（Affix）：只出现在一个单词的词首或词尾；本身不具有实际意义，仅具有语法意义。
		- 屈折词缀（Inflectional Affix）：顾名思义，此类词缀构成单词的屈折形式。
			- 名词复数标记：-s /-es;
			- 名词属格标记：-'s; 
			- 第三人称动词单数现在时标记：-s /-es;
			- 动词过去时标记：-ed
			- 动词-ed分词和-ing分词标记：-ed, -ing
			- 形容词和副词比较级和最高级标记：-er, -est
			- etc.
		- 派生词缀（Derivational Affix）：可以用来构成派生词
			- 前缀（Prefix）
				- **anti**-war
				- **post**war
				- **co**-exist
			- 后缀（Suffix）
				- Marx**ist**
				- move**ment**
				- care**less**
	- 组合形式（Combining Form）：即可以作为词根（称为粘附词根）构成一种特殊的派生词（原始派生词），也可以与单词或其他组合形式相结合构成复合词。可出现在词首，词尾，或词中。例如：arch-, auto-, bi(o)-, -crat, -cracy, -dict, -duct, geo-, -graph, -graphy, hydro-, -logue, -log, micro-, mono-, multi-, -phone, pseudo-, psych(0)-, -scope, techn(o)-, tele-, 等。都是一些源自希腊文、拉丁文等古典文字的外来词。
		- 作为粘附词根构成原始派生词：
			- -ceive
				- receive
				- conceive
				- perceive
			- -duct
				- conduct
				- misconduct
				- semiconductor
			- -dict
				- prediction
				- contradictory
				- dictate
		- 作为词素与其他单词组成复合词：（像是前缀，但其实不是）
			- arch-
				- archenemy
				- archbishop
			- mini-
				- minibus
				- miniskirt
			- mal-
				- maltreat
				- maladjusted
		-  作为词素与其他组合形式构成「新古典复合词」：
			- bio + logy = biology
			- micro + phone = microphone
			- mono + logue = monologue
			- tele + scope = telescope
			- electro + cardio + gram = electrocardiogram
			- cine + angio + cardio + graphy = cineangiocardiography


### 2️⃣ Word & 3️⃣ Word Phrase
根据单词的构词法，英语的单词分为简单词，派生词，复合词：
- Simple Word
- Derivative Word
- Compound Word

根据单词的语法功能，分为封闭词类和开放词类：
- Closed Class
- Open Class
#### 👉 N. & Noun Phrase (名词)

#### 👉 Determiner (限定词)
> 🔗《新编英语语法教程》 - 新世纪高等院校英语专业本科生系列教材

限定词（Determiner）是在名词词组中对名词中心词起特指（Specific Reference）、类指（Generic Reference）以及表示确定数量（Definite Quantity）和非确定数量（Indefinite Quantity）等限定作用的词类。各词词组除有词汇意义外，还有其所指意义（Referential Meaning），是特指（即指特定的对象），还是类指（即泛指一类的人或物）；是有确定的数量，还是没有确定的数量。能在名词词组中表示这种所指意义的词类就是限定词。

英语的限定词包括：
- 定冠词 （Definite Article），不定冠词 （Indefinite Article），零冠词（Zero Article）
- 物主限定词（Possessive Determiner）: my, your, his, her, our, your, their, one's, its
- 名词属格（Genitive Noun）：John‘s, my friend's
- 指示限定词（Demonstrative Determiner）：this, that, these, those, such
- 关系限定词（Relative Determiner）：whose, which
- 疑问限定词 （Interrogative Determiner）：what, which, whose
- 不定限定词（Indefinite Determiner)：no, some, any, each, every, enough, either, neither,  all, both, half, several, many, much, (a) few, (a) little, other, another
- 基数词（Cardinal Numeral）和序数词（Ordinal Numeral）
- 倍数词（Multiplicative Numeral） 和分数词（Fractional Numeral）
- 量词 (Quantifier)： a lot of, lots of, plenty of, a great / good deal of, a large / small amount / quantity of, a great / large / good number of 等。

限定词与名词词组中心词之间有着某种固定的搭配关系；如果名词之前带有两个或两个以上的限定词，则限定词与限定词之间也有某种固定的搭配关系。本讲主要解决这两种搭配关系问题。
#### 👉 Pronoun (代词)
##### Personal Pronoun
##### Possessive Pronoun
##### Reflexive Pronoun
##### Demonstrative Pronouns
##### Reciprocal Pronoun
##### Indefinite Pronoun

#### 👉 V. & Verb Phrase (动词)
##### Classification of (Predicate) Verbs
英语动词按其在构成动词词组中所起的作用分为主动词（Main Verb）-- 又称实义动词（Notional Verb）-- 和助动词（Auxiliary Verb）两大类。
动词词组是以主动词为中心词的词组，分为简单动词词组和复杂动词词组。简单动词词组只由一个主动词构成，而复杂动词词组由一个或一个以上的助动词加主动词构成。
- 主动词
	- 主动词的分类
		- 按是否必须跟有补足成分（Complement）以及必须跟有什么样的补足成分如宾语、补语、状语等，分为及物动词（Transitive Verb），不及物动词（Intransitive Verb），连系动词（Linking Verb）;
		- 按主动词的构成，分为单词动词（），词组动词（）。单词动词是只由一个单词构成的主动词，而词组动词是由两个及以上个单词构成的主动词；
			- 即：单词动词 = 1x单词，词组动词 = 单词 + 单词 + ... ；不论单词动词还是词组动词，都是一个主动词；
			- 注意和动词词组的区别：简单动词词组 = 主动词，复杂动词词组 = 若干助动词 + 主动词
		- 按其词汇意义，可分为动态动词和静态动词。动态动词是表示物体运动状态的动词，而静态动词是表示一种相对静止状态的动词。
		- 按其屈折变化规律，分为规则动词和不规则动词；
- 助动词
	- 助动词的分类：
		- 基本助动词：be, do, have
		- 情态助动词：can /could, may /might, will /would, shall /should, must, ought to, dare /dared, need, used to
			- 情态助动词表示情态意义，过去时形式不一定就是表示过去的时间。情态助动词不能重叠使用；==随后的主动词无一例外地是不带to的不定式（即动词原形）。== (这种情况下，主动词属于非限定动词，这与大部分谓语动词是限定性动词的情况相反：此时谓语动词是由情态助动词 + 主动词/非限定动词构成，即一个不是限定动词的复杂动词词组)
		- 半助动词
	- 助动词的缩略形式
		- 否定缩略
		- 肯定缩略


英语动词按其是否「在人称和数的方面与主语保持一致并具有时态」，可将动词分为限定性动词（Finite Verb）和非限定性动词（Non-Finite Verb）。
- 限定性动词：原形（Base Form） + 现在时（Present Tense）+ 过去时（Past Tense） + 动词单复数（）
- 非限定性动词：不定式（Infinitive Verb） + -ing分词（-ING Participle） + -ed分词（ -ED Participle）

> ⚠️注意：
> 非限定性动词在以往被广大地称作“非谓语动词”，意为该类动词不做谓语。但是根据非限定性从句的定义，非限定性从句就是用非限定性动词作谓语动词的从句。因此，本书不使用“非谓语动词”这个概念。
##### Properties of Verbs
###### Verb Tense（时）
###### Verb Aspect（体）
###### Verb Voice（态）& Passive Meaning Expression

#### 👉 Adj. & Adjective Phrase (形容词)
##### Gradable Adjective

#### 👉 Adv. & Adverb Phrase (副词)

#### 👉 Prep. & Prepositional Phrase (介词)
#### Special Usages of Words and Word Phrases
##### Modification
##### Substitution
##### Ellipsis
##### Emphasis

### 4️⃣ Clause

按照层次性理念，分句是建筑在词组基础上的，介乎词组与句子之间的语法单位；从逻辑意义上看，分句又是形成“主谓结构”，从而带有述说性的词组序列。
#### Structure of Clause
```
Clause = Subject + Predicate
e.g. S-V-O

						Sentence
						   |
		 ----------------------------------------
		 |										|
	  Subject								Predicate      --> Clause Structure
		 |										|
		 |								-------------------
		 |								|				  |
	  Subjetct					       Verb			   Object  --> Clause Types 
		 |								|				  |
	  Noun Phrase					Verb Phrase		   Noun Phrase
		 |								|				  |
	------------					----------- 	 -----------
	|			|					|		  |		 |			|
	The		producers			are able to supply	 a small part of our needs.


Basic Clause Types:
- S-V-C
- S-V
- S-V-O
- S-V-o-O
- S-V-O-C
```
#### Types and Functions of Clause
Independent Clause /Dependent (Subordinate) Clause
Simple Clause /Complex Clause
Main Clause /Subordinate Clause
Finite Clause /Non-finite Clause /Verbless Clause
##### Relative Clause (关系分句) (旧作定语从句)
在我国的语法书中，此种分句曾长期叫做“定语从句”，这是按照此种句式的主要用法命名，但是这种分句除作名词修饰语之外，还可在特定语境中起到并列分句和状语分句的作用。如果一律视为“定语从句”，则往往名不副实，在理解上起到误导作用。试观察下列诸例：
- She passed the letter to Moriarty, **who passed it to on to me**.
	- (= ... and Moriarty passed it on to me.)
- I dropped the saucepan, **which knocked over the eggs, which went all over the floor**.
	- (= ... and it knocked ... and they went ...)
- In 1906, however, Pierre, **who was crossing a road**, was run over and killed.
	- (= when he was crossing the road, ...)
- But he would have to be careful not to offend Crass, the foreman, **who could give him the sack at any time**.
	- (= ...because he could give him the sack at any time.)
- Shortly after Lincoln took office, the Southern States rebelled and set upa state of their own, **where they would be free to keep Negroes as slaves**. 
	- (= .. so that they would be free to keep Negroes as salves.)
#### Basic Clause Types ⭐
所谓基本句型就是分句结构的几种基本“格局”，也就是千变万化的句子的结构雏形。正是这些有限的分句结构雏形能够衍生出无限的实际中使用的句子。现代英语的基本句型有五种：
- S-V-C
- S-V
- S-V-O
- S-V-o-O
- S-V-O-C
#### Clause Element
Central Element
- Subject
- Predicate Verb /Predicator
- Object
- Complement
	- Subject Complement
	- Object Complement
- Adverbial
	- Depictive Adverbial /Adjunct
Peripheral Element
- Adverbial
	- Stance Adverbial /Disjunct
	- Linking Adverbial /Conjunct
#### Subject-Verb Agreement (Concord)


### 5️⃣ Sentence
句子是最高一级的语法单位，也是人们表达思想进行交际的基本语言单位。作为人们表达思想的基本语言单位，句子具有以下两个特征：一是句子能在交际中单独使用；二是句子能够表达相对完整的意思，并在一定上下文或语境中发挥一定的交际作用。句子建筑在分句基础上，它可以包括一个或一个以上的分句。当句子只包括一个分向时，句子和分句是没有区别的；但是，句子又不同于分句：作为最高一级的语法单位，句子的结构形式是无限的，而分向的结构形式却是有限的。==因此，当我们讲到“句子成分”、“基本句型”时，指的都是分句的成分和基本的分句句型。==

```
The larying structure of a sentence:

							Sentence
							   |
							(Clause)	 
			----------------------------------------
			|										|
		  Subject								Predicate
			|										|	   
			|						   ---------------------------------
			|						   |							   |
		Noun Phrase				   Verb Phrase					  Prep Phrase
			|						   |							   |		
	---------------			--------------------			-----------------
   ｜   		  |			|          |       |			|      |	    |
  Det	  		  N			Aux 	  Adv	   MV			Prep   Det	    N
These    under·graduate·s	are	   rapid·ly	  improv·ing	in	  their	 writ·ing
```
#### Type of Sentences
按句子的构成，句子分为完全句和不完全句：
- Full Sentence
	- Simple Sentence
	- Compound Sentence
	- Complex Sentence
	- Compound-complex Sentence
- Minor Sentence

按句子的交际功能，句子分为陈述句，疑问句，祈使句，和感叹句：
- Declarative Sentence
	- Special Types of Declarative SentenceL
		- Existential Sentence
		- Conditional Sentence
		- Quote
- Question Sentence
- Imperative Sentence
- Exclamation Sentence

其他一些语法结构：
- Negative Structure
- Parallel Structure
- Subordinate Structure
- Hypothetical Meaning Expression & Subjunctive Mood
- Passive Voice & Passive Meaning Expression
#### Components of Sentence

#### Structure of Sentence



## English Textual Hierarchy
### Sentence


### Sentence Group


### Text



## Ref
