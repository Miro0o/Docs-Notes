...
- [Shared Human Factor](../../Human-Factor%20in%20LLM%20&%20Software.md): developer and team behavior, correctness and review, maintenance, performance, operations, trust, learning, adoption, and work design across LLM/software systems.
...
The [BibTeX file](LLM-Software-Research-Dossier-2026.bib) contains every record assigned to this dossier by the frozen [shared corpus](../Literature-Corpus/README.md), plus clearly labeled supplementary records retained from the earlier dossier. The [canonical corpus map](Canonical-Corpus-Map.md) is the row-level audit index; topic pages remain the interpreted reading route and may cross-link rather than repeat full rows. `formal-venue`, `frontier-preprint`, and supplementary keywords preserve evidence class. Prefer DOI records or official proceedings pages, retain exact venue status, and never upgrade a submission or preprint to “accepted” without an official program or proceedings record.
