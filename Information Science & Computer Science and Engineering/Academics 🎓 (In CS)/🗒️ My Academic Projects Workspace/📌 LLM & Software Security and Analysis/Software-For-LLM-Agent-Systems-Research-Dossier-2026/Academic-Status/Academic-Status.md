...
| --- | --- | --- | --- |
| PL | PLDI/PACMPL, POPL, OOPSLA, ICFP | Foundational LMQL at PLDI 2023; MTP and Opportunistically Parallel Lambda Calculus at OOPSLA 2025. | ICFP/OOPSLA/POPL 2026 for additional typed effects, agent languages, and semantics. |
| AI/ML/NLP | ICLR, ICML, NeurIPS, ACL; MLSys where the programming/runtime contribution is central | DSPy, SGLang, DOMINO, APPL, EvoMAC, DEI, AgentBoard, τ-bench, ITBench, Agent Data Protocol, DoVer, AgenTracer. | NeurIPS 2026 and final ICLR/ICML 2026 metadata. |
| SE | ICSE, FSE/PACMSE, ASE, ISSTA; ICSA for architecture | FSE 2025 prompt-programming study, ASE 2025 Watson, ICSE 2025 SEIP field map, ICSA 2025 NIER architecture, ISSTA 2026 accepted agent testing. | Normalize ISSTA 2026 DOI/pages after proceedings and continue AIware evidence with venue labels intact. |
| OS/systems | OSDI, SOSP, NSDI; EuroSys and ATC when workflow structure is primary | Parrot at OSDI 2024, Murakkab at OSDI 2026, and Agentix at NSDI 2026. | Check systems venues for application-level agent scheduling, dataflow, and SLO work; reject generic serving-only papers. |
...
Coverage means that directly relevant work was checked, not that every venue must contribute a paper.
...
Prefer official proceedings and DOI records. Official conference programs support `Accepted` or `Official program`. Preprints remain `Frontier`; a submission label is never treated as acceptance. Product documentation and standards belong in [Non-Academic Status](../Non-Academic-Status.md).
